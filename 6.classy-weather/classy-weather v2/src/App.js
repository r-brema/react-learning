...
class App extends React.Component {
  ...
  getWeather = async () => {
    if (this.state.weather.length < 2) return this.setState({ weather: {} });
    try {
      // fetch location (geocoding)
  ...
  render() {
    return (
      <div className="app">
        ...
        {this.state.location === "" ? (
          <p>Search for Loaction to see Weather</p>
        ) : (
          this.state.isError && <p>{this.state.isError} </p>
        )}

        {!this.state.isError && this.state.weather.weathercode && (
          <Weather
